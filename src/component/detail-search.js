window.onload = function () {
    ...
    // Get data from API
    fetch('https://comptour-be.vercel.app/api/tourist-attractions/get-detail?taid=' + taid, requestOptions)
        .then(response => response.json())
        .then(data => {
            const attractions = data.data;

            document.getElementById('province').innerText = attractions.province + ', ' + attractions.regency + ', KECAMATAN ' + attractions.district
            document.getElementById('description-header').innerText = attractions.description
            document.getElementById('description').innerText = attractions.description

            // Create a map object
            const map = L.map('map').setView([attractions.longtitude, attractions.latitude], 6);
            ...
            L.marker([attractions.longtitude, attractions.latitude]).addTo(map)
